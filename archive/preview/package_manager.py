...
def display_help():
    ...
    print("  list_version               - Show current LightPack version")
    print("  systeminfo                 - Show system information")
    print("  info                       - Show general info about LightPack")
    print("  clearrepos                 - Clear all community repositories")
    print("  backup                     - Backup installed packages")
    print("  restore                    - Restore packages from backup")

def info():
    """Display general information about LightPack."""
    print(f"LightPack version: {VERSION}")
    print("A package manager for LightPack-based systems.")
...
def clear_repos():
    """Clear all community repositories."""
    if os.path.exists(REPO_FILE):
        os.remove(REPO_FILE)
    print("All community repositories cleared.")
...
def backup():
    ...
    with tarfile.open(backup_file, "w:gz") as archive:
        for file in os.listdir(INSTALL_DIR):
            if file.endswith(".py") and not file.endswith("_install.py"):
                archive.add(os.path.join(INSTALL_DIR, file), arcname=file)
